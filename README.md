...
1. Allow GPS. The yellow machine marker is **this phone**. Big top bar = **who you are** (Dozer / Excavator / Water). Tap it to switch role. GPS + LIVE sit under that identity bar. **No job code** — everyone on the Pages URL shares one open site.
...
Deploys from **main** `/` (static HTML, `.nojekyll`). Push to `main` updates the live site. Assets are cache-bumped (`?v=7`, service worker `onpad-v7`).
